...
void SetRoutingEntries(){
	...
	for (auto i = nextHop.begin(); i != nextHop.end(); i++){
		...
		for (auto j = table.begin(); j != table.end(); j++){
			...
			// Use the canonical server IP instead of relying on interface/address slot layout.
			Ipv4Address dstAddr = serverAddress[dst->GetId()];
...
{
	// Force link DropTailQueue<Packet> registration
	(void) DropTailQueue<Packet>::GetTypeId();
	clock_t begint, endt;
	begint = clock();
#ifndef PGO_TRAINING
	if (argc > 1)
	...
	for (uint32_t i = 0; i < node_num; i++){
		if (node_type[i] == 0)
			n.Add(CreateObject<Node>());
				...
				n.Add(sw);
				sw->SetAttribute("NodeType", UintegerValue(1));
